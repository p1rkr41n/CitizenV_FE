...
const state = {
    ...
    socketConnected: 0,
    messages: [],
    chatUsers: [], // {id: '', status: '', clientId, 'device'}
}
...
const getters = {
    ...
    getStatusByName: (state) => (id) => {
        const user = state.chatUsers.find((item) => item.id === id)
...
const actions = {
    initSocket({ commit, dispatch, rootState }) {
        ...
        socket.on('connect', () => {
            const user = { id: rootState.auth.id }
        ...
        socket.on('message', ({ id, text, createdAt }) => {
            dispatch('pushMessage', { id, text, createdAt })
        })
    ...
    pushMessage({ commit, dispatch }, message) {
        dispatch('updateNotification', `New Messsage from ${message.id}: ${message.text}`)
...
const mutations = {
    ...
    UPDATE_USER_LIST(state, users) {
        users.forEach((user) => {
            const index = state.chatUsers.findIndex((item) => item.id === user.id)
            if (index === -1) {
